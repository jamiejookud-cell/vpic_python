...
from define import *
from custom_advanced_function import *

TIME_START = datetime.now()

# ux, jx, jx ux, uz, jz
terms = [box_frame_zero_array.copy(),box_frame_zero_array.copy(),box_frame_zero_array.copy(),box_frame_zero_array.copy(), box_frame_zero_array.copy()]
with (Progress() as progress):
    task = progress.add_task(f"[white] processing...", total=folder_count)
    # Catching corrupted files.
    for COUNT, FILENAME in enumerate(folders):
        data = read_timestep(filename=FILENAME)
        if data is None:
            folder_count -= 1
            progress.update(task, advance=1)
            dump_process(f'Found corrupted folder {FILENAME}, folder_count = {folder_count}.')
            continue
        ...
        if IS_CALCULATING_SHOCK_VELOCITY:
            peak_value: float = get_shock_peak_index(rho_i)
            x_peak: float = peak_value * dx_de  # [length unit]
            t_peak: float = current_timestep * dt_wpe  # [time unit]
            shock_distances.append(x_peak)
            shock_times.append(t_peak)
...
if IS_EXPORT_DATA_TO_CSV:
    export_csv(jx_e_prime, f'jx_e_{SPECIFIC_TIMESTEP}')
    export_csv(jy_e_prime, f'jy_e_{SPECIFIC_TIMESTEP}')
    export_csv(jz_e_prime, f'jz_e_{SPECIFIC_TIMESTEP}')
    export_csv(rho_e_prime, f'rho_e_{SPECIFIC_TIMESTEP}')
    export_csv(cex_prime, f'ex_{SPECIFIC_TIMESTEP}')
    export_csv(cey_prime, f'ey_{SPECIFIC_TIMESTEP}')
    export_csv(cez_prime, f'ez_{SPECIFIC_TIMESTEP}')
    export_csv(cbx_prime, f'bx_{SPECIFIC_TIMESTEP}')
    export_csv(cby_prime, f'by_{SPECIFIC_TIMESTEP}')
    export_csv(cbz_prime, f'bz_{SPECIFIC_TIMESTEP}')
